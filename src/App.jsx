...
const SCAN_MODES = {
  merged: {
    ...
    prompt:
      "לפניך צילום עמוד מספר קודש (זוהר / תיקונים / דף מפורש). פענח את העמוד והרכב אותו מחדש כך: כל פסקת מקור (ארמית, לרוב עם אות סימון כמו א) ב) או קפח)) — ומיד אחריה הפירוש/התרגום העברי שלה מהעמוד (כגון תרגום, הסולם), בשורה שמתחילה ב'פירוש: '. שמור על סימוני הפסקאות המקוריים. דלג על כותרות עמוד, מספרי עמוד ומראי-מקומות שוליים (מסורת הזוהר, חילופי גרסאות) — אל תכלול אותם. תקן שברי מילים שנוצרו מהצילום לפי ההקשר בלבד. חשוב מאוד: אם מילה אינה קריאה בבירור — כתוב אותה כפי שהיא נראית ואל תנחש מילים שאינן בעמוד.",
  },
  layers: {
...
async function smartScanImages(files, mode, onProgress) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      t = await scanPiece(spec, imgToJpegBase64(img, 1800), "", 2600);
    } catch (e1) {
