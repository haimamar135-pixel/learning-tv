...
export default function LearningTV() {
  ...
  const createBook = async () => {
    ...
    if (!t || t.length < 40) {
      setError("הדבק טקסט של לפחות כמה משפטים כדי להתחיל.");
      return;
    }
    const chapters = splitToChapters(t);
    const title = titleRef.current?.value?.trim() || chapters[0].title || "ספר ללא שם";
    const nb = {
            ...
            <div className="screen-body">
              ...
              {view === "intake" && (
                <div className="intake">
                  <p className="intake-lead">
                    הדבק ספר, פרק או מאמר — והמסך יהפוך אותו לסדרת פרקים עם ערוצי למידה: סיכום, מושגים, מפת חשיבה, מבחן ועוד. ההתקדמות נשמרת, כך שאפשר ללמוד ספר שלם לאורך זמן.
                  </p>
                  ...
                  <div className="btn-row">
                    ...
                    {index.length > 0 && (
                      <button className="ghost-btn" onClick={backToLibrary}>לספרייה</button>
                    )}
                  </div>
                </div>
              )}
              ...
              {view === "guide" && book && (
                <div className="guide">
                  ...
                  {nextUnfinished >= 0 && (
                    <button className="broadcast slim" onClick={() => openChapter(nextUnfinished)}>
                      ▸ המשך לימוד — {book.chapters[nextUnfinished].title}
                    </button>
                  )}
