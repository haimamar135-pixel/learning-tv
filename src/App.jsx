...
export default function LearningTV() {
  ...
  const onFilePicked = async (e) => {
    ...
    if (fileRef.current) fileRef.current.value = ""; // אפשר לבחור שוב את אותו קובץ
    if (!file) return;
    setError(null);
    ...
    try {
      ...
      if (!text || text.length < 40) {
        setError("לא נמצא מספיק טקסט בקובץ.");
        setFileBusy(null);
        return;
      }
      // שם ברירת מחדל משם הקובץ (בלי הסיומת)
                ...
                <div className="intake">
                  ...
                  <div className="upload-box">
                    ...
                    <span className="upload-hint">
                      הקובץ נקרא במחשב שלך בלבד ולא נשלח לשום מקום. PDF סרוק (צילום) יזוהה ותוצג הודעה.
                    </span>
                  </div>
